...
import com.zylo.resumecraft.security.JwtTokenProvider;
import com.zylo.resumecraft.service.AuthService;
...
public class AuthServiceImpl implements AuthService {
    ...
    public AuthServiceImpl(UserMapper userMapper, UserAuthIdentityMapper userAuthIdentityMapper,
                           ...
                           KeyValueStore keyValueStore, MailService mailService) {
    ...
    public TokenDTO register(RegisterDTO dto) {
        ...
        // 校验验证码
        if (!isDevelopment() && !verifyCode(normalizedEmail, dto.getVerificationCode())) {
        ...
        // 删除已使用的验证码
        keyValueStore.delete("verify:" + normalizedEmail);
    ...
    public TokenDTO refreshToken(String refreshToken) {
        ...
        var storedToken = keyValueStore.get("refresh:" + userId + ":" + jti);
        if (storedToken == null) {
            throw new BusinessException(ResultCode.REFRESH_TOKEN_EXPIRED);
        }

        // 删除旧的 refresh token
        keyValueStore.delete("refresh:" + userId + ":" + jti);
    ...
    public void logout(Long userId, String accessToken) {
        ...
        if (remainingSeconds > 0) {
            keyValueStore.set("blacklist:" + jti, "1", remainingSeconds, TimeUnit.SECONDS);
        }

        // 删除 refresh token（通过模式匹配）
        var keys = keyValueStore.keys("refresh:" + userId + ":*");
    ...
    public void sendVerificationCode(String email) {
        ...
        // 检查发送频率（60 秒内只能发一次）
        var lastSent = keyValueStore.get("rate:" + normalizedEmail);
        if (lastSent != null) {
        ...
        // 存储验证码到 Redis（5 分钟有效）
        keyValueStore.set("verify:" + normalizedEmail, code, 5, TimeUnit.MINUTES);

        // 设置发送频率限制（60 秒）
        keyValueStore.set("rate:" + normalizedEmail, "1", 60, TimeUnit.SECONDS);
    ...
    public boolean verifyCode(String email, String code) {
        var storedCode = keyValueStore.get("verify:" + normalizeEmail(email));

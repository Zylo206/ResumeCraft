...
const AI_LOG_PREFIX = '[ResumeCraft AI]'
const STREAM_API_BASE_URL = import.meta.env.VITE_DESKTOP === 'true'
  ? ''
...
function buildStreamApiUrl(path: string) {
  ...
  return `${window.location.origin}${STREAM_API_BASE_URL}${path}`.replace(/([^:]\/)\/+/g, '$1')
}
...
export const resumeApi = {
  ...
  reorderModules: (resumeId: number, moduleIds: number[]) =>
    client.put<ApiEnvelope<ResumeModule[]>>(`/resumes/${resumeId}/modules/reorder`, { moduleIds }),
